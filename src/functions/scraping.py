...
async def get_site_content(URL: str) -> Optional[Any]:
    ...
    # 1) Agar ishlaydigan proxy bo‘lsa — ulardan foydalanamiz
    if proxies:
        for proxy in proxies:
            try:
                ...
                print(f"⚠️ Proxy ishlamadi: {proxy} ({e})")
                continue

    # 2) Agar hech biri ishlamasa — to‘g‘ridan-to‘g‘ri ulanib ko‘ramiz
    try:
        async with aiohttp.ClientSession(headers=headers) as session:
...
class ProxyManager:
                ...
                async with session.get(url, headers=headers, timeout=20) as resp:
                    ...
                    for row in rows[:max_rows]:
                        try:
                            ...
                            # script ichidan port decode qilish
    ...
    def _decode_port_from_script(self, script_text: str, var_map: dict) -> Optional[int]:
        """Script ichidagi XOR asosida portni dekodlash."""
        try:
            ...
            parts = []
            for a, b in ops:
                v1 = var_map.get(a) or int(re.search(r'\d+', a).group(0)) if re.search(r'\d+', a) else 0
                v2 = var_map.get(b) or int(re.search(r'\d+', b).group(0)) if re.search(r'\d+', b) else 0
    ...
    async def get_working_proxies(self, max_proxies: int = 10, max_tests: int = 30) -> List[str]:
        """Parallel ravishda ishlaydigan proxy'larni topish."""
        if not self.proxies:
        ...
        results = await asyncio.gather(*(sem_test(p) for p in test_proxies))
...
if __name__ == "__main__":
    async def test():
        ...
        data = await get_site_content(url)
        print("\nNatija:")
        print(data if data else "Hech narsa qaytmadi")
