...
async def referral(user=Depends(current_user), db=Depends(get_db)) -> ReferralResponse:
...
@router.get("/stats")
async def referral_stats(user=Depends(current_user), db=Depends(get_db)) -> dict[str, int | bool | str]:
    settings = get_settings()
    user_id = int(user["user_id"])
