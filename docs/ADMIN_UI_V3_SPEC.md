...
2b. **Asosiy amal sarlavhada, Telegram MainButton ishlatilmaydi** (2026-09-22 da o'zgartirildi: MainButton bizning pastki panelimiz ostida yana bir to'liq kenglikdagi ko'k qator chizib, ekrandan joy yeydi va sarlavhadagi amalni takrorlaydi). Har ekranning yagona asosiy amali sarlavhaning o'ng burchagida ixcham `Button size="sm"`; shell mount bo'lganda MainButton majburan yashiriladi. Tasdiq va xato holatlarida `HapticFeedback` (`haptic()` yordamchisi).
2c. **Fullscreen safe-area.** BotFather orqali Mini App fullscreen sozlangan: Telegram o'zining ‹ / ⋮ / ✕ tugmalarini sahifa ustida suzuvchi qilib chizadi. Admin sarlavhasi `--admin-top-inset` (`max(content-safe-area-top, safe-area-top)`, fullscreen'da kamida 52px) qadar pastdan boshlanadi, aks holda tugmalar ostida qolib ketadi. Faqat admin panelga tegishli: ilovaning `Layout`i o'z padding'ini allaqachon qo'llaydi.
